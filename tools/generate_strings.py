...
SPECIAL_PATH = (
    ROOT / "custom_components" / "thessla_green_modbus" / "options" / "special_modes.json"
)
...
def build():
    ...
    parity_map = {
        "brak": ("none", "None", "Brak"),
        "parzysty": ("even", "Even", "Parzysta"),
        "nieparzysty": ("odd", "Odd", "Nieparzysta"),
    }
    parity_en = {
...
def write():
    ...
    def apply(obj, lang):
        ...
        obj["services"]["set_special_mode"]["fields"]["mode"]["selector"]["select"]["options"] = (
            opts["special_mode"][0 if lang == "en" else 1]
        )
