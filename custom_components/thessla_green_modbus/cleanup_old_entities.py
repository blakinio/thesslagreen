...
def backup_file(file_path: Path) -> Path:
    ...
    _LOGGER.info("✅ Backup utworzony: %s", backup_path)
    return backup_path
        ...
        if cache_path.exists():
            try:
                if cache_path.is_dir():
                    ...
                    cache_path.unlink()
                _LOGGER.info("🧹 Usunięto cache: %s", cache_path)
                cleaned = True
            except Exception as exc:
                _LOGGER.warning("⚠️  Nie można usunąć cache %s: %s", cache_path, exc)
    
    if not cleaned:
        _LOGGER.info("ℹ️  Nie znaleziono cache do wyczyszczenia")
...
def main():
    ...
    if not config_dir:
        _LOGGER.error("❌ Nie można znaleźć katalogu konfiguracyjnego Home Assistant")
        _LOGGER.error("Sprawdź czy znajdujesz się w odpowiednim katalogu lub HA jest zainstalowany")
        sys.exit(1)

    _LOGGER.info("📁 Znaleziono konfigurację HA: %s", config_dir)
    _LOGGER.info("")
    ...
    _LOGGER.info("\nWynik: %s/%s zadań zakończonych pomyślnie", success_count, total_count)

    if success_count == total_count:
        _LOGGER.info("\n🎉 Czyszczenie zakończone pomyślnie!")
        _LOGGER.info("💡 Możesz teraz bezpiecznie zrestartować Home Assistant")
    else:
        _LOGGER.warning("\n⚠️  Niektóre zadania wymagają uwagi")
        _LOGGER.warning("📝 Sprawdź i popraw wskazane problemy przed restartem HA")

    # Pokaż instrukcje następnych kroków
    _LOGGER.info("\n🔄 NASTĘPNE KROKI:")
    _LOGGER.info("1. Sprawdź powyższe ostrzeżenia (jeśli są)")
    _LOGGER.info("2. Zrestartuj Home Assistant")
    _LOGGER.info("3. Sprawdź czy integracja działa poprawnie")
    _LOGGER.info("4. Usuń niepotrzebne backupy z .storage/ (opcjonalnie)")
