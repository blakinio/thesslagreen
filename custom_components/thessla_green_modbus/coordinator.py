    ...
    def _compute_register_groups(self) -> None:
        ...
        for key, names in self.available_registers.items():
            ...
            self._register_groups[key] = group_reads(
                addresses, max_block_size=self.max_registers_per_request
            )
            ...
            try:
                ...
                if definition.function == "03":
                    if encoded_values is not None:
                        limit = self.max_registers_per_request
                        response = None
                        for offset in range(0, len(encoded_values), limit):
                            chunk = encoded_values[offset : offset + limit]
                            response = await self._call_modbus(
