            ...
            select_mappings.setdefault(
                register,
                {
                    ...
                    "states": __import__(
                        ...
                        fromlist=["PERCENT_10_SELECT_STATES"],
                    ).PERCENT_10_SELECT_STATES,
                },
            )
        else:
...
def route_enum_or_min_max(
    ...
    if reg.enum and not (reg.extra and reg.extra.get("bitmask")):
        target, payload = classify_enum_mapping(
            register, reg.enum, access, switch_keys, binary_keys, select_keys
        )
        route_enum_mapping(
            target,
            register,
        ...
        return
    target, payload = classify_min_max_mapping(
        register,
        access,
        min_val,
        max_val,
