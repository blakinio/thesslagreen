...
class ThesslaGreenDeviceScanner:
    ...
    def _analyze_capabilities(self) -> DeviceCapabilities:
        ...
        if any(reg in holdings or reg in inputs for reg in basic_control_regs):
        ...
        for reg in flow_regs:
            if reg in inputs or reg in holdings:
                caps.flow_sensors.add(reg)
    ...
    def _log_skipped_ranges(self) -> None:
        ...
        for reg_type, addrs in self.failed_addresses["modbus_exceptions"].items():
            if addrs:
                addresses = ", ".join(str(addr) for addr in sorted(addrs))
                _LOGGER.warning("Failed to read %s at %s", reg_type, addresses)

        for reg_type, addrs in self.failed_addresses["invalid_values"].items():
            if addrs:
                addresses = ", ".join(str(addr) for addr in sorted(addrs))
                _LOGGER.debug("Invalid values for %s at %s", reg_type, addresses)
