...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):  # type: ignore[call-arg]
    ...
    def __init__(self) -> None:
        ...
        self._reauth_entry_target_id: str | None = None
    ...
    async def async_step_reauth(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if self._reauth_entry_target_id is None:
            self._reauth_entry_target_id = entry.entry_id if entry else None
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            if self.hass is None or self._reauth_entry_target_id is None:
                _LOGGER.error("Cannot complete reauth - missing Home Assistant context")
                return self.async_abort(reason="reauth_failed")

            entry = self.hass.config_entries.async_get_entry(self._reauth_entry_target_id)
            if entry is None:
                _LOGGER.error(
                    "Reauthentication requested for missing entry %s",
                    self._reauth_entry_target_id,
                )
