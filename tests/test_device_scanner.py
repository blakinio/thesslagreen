...
async def test_verify_connection_close_type_error_does_not_mask_connection_failure():
    """Verify close() TypeError does not mask connection failure."""
    scanner = await ThesslaGreenDeviceScanner.create("192.168.3.17", 8899, 10)
    fake_client = MagicMock()
    fake_client.connect = AsyncMock(return_value=False)
    fake_client.close = MagicMock(side_effect=TypeError("close failed"))
